...
def test_server_prompts_tab_route_preserves_server_name(monkeypatch) -> None:
    app = create_app()
    server_id = uuid4()
    version_id = uuid4()
    captured: dict[str, str] = {}
    ...
    async def get_prompts_tab(_session, server_name):
        captured["server_name"] = server_name
        return {
            "server": {
                "id": str(server_id),
    ...
    app.dependency_overrides[get_db_session] = fake_session
    monkeypatch.setattr(router, "get_server_prompts_tab", get_prompts_tab)

    response = TestClient(app).get(
        "/api/v1/mcp/servers/io.github.example/weather/tabs/prompts"
    )
